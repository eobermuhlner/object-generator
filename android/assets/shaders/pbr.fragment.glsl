...
void main() {
        ...
        vec3 ambientLight = vec3(0.0);
	        ...
	        vec4 pos = v_pos;
                ...
                #ifdef pbrFlag
                    #if defined(specularTextureFlag) && defined(specularColorFlag)
                        vec3 mga = texture2D(u_specularTexture, v_specularUV).rgb * u_specularColor.rgb;
                    #elif defined(specularTextureFlag)
                        vec3 mga = texture2D(u_specularTexture, v_specularUV).rgb;
                ...
                for (int i = 0; i < numDirectionalLights; i++) {
                    ...
                    vec3 value = u_dirLights[i].color * NdotL;
                        ...
                        #ifdef pbrFlag
                            lightDiffuse += value * (1.0 - metal);
                            lightSpecular += value * pow(halfDotView, gloss) * metal;
                        #else // ! pbrFlag
                            lightDiffuse += value;
